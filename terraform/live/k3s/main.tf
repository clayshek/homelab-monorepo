...
locals {
  ...
  k3sserver_clone = "template-ubuntu-18-04-5-pve1" 
  ...
  k3sworker_clone = "template-ubuntu-18-04-5-pve2" 
...
resource "null_resource" "configuration" {
  ...
  provisioner "local-exec" {
    command = "ssh-keygen -f ~/.ssh/known_hosts -R ${local.k3sserver_ip_address}"
  }
